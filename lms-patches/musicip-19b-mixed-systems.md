...
- **Affected:** LMS's native MusicIP integration — `Slim::Plugin::MusicMagic::Plugin::getMix()` (browse-by-mix, Mood Mix, instant mix) and the library scan/import path in `Importer.pm`.
...
## ⚠️ Warning: MusicIP 1.9.b on mixed systems — silent LMS database corruption
...
| `Plugin.pm` regex order | Prefix strip runs after backslash conversion → `Z:/music/...` |
...
This has been confirmed in practice by multiple users — a partial fix or database repair does not work reliably.

### Required configuration — all or nothing
...
MusicIP 1.8 is a native Linux binary. It communicates directly in Linux paths — no Wine, no Windows path notation, no translation layer. On a pure Linux setup with 1.8, none of this applies.
